...
//         html: z.string().describe("The HTML content of the resume which can be converted to PDF using using any libraray like puppeteer")
//     })

//     const prompt = `Generate html content for a candidate to create resume PDF for a candidate with the following details:
//                     Resume: ${resume}
//                     selfDescription : ${selfDescription}
//                     jobDescription: ${jobDescription}
//                     `
...
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
    ...
    const resumepdfSchema = z.object({
        html: z.string().describe(
            "Complete valid HTML content for a professional resume that can be converted to PDF using Puppeteer"
        )
    });

    const prompt = `
Generate a complete professional HTML resume for the candidate.

IMPORTANT:
- Return complete valid HTML.
- Include <!DOCTYPE html>, html, head, style, and body.
- Make the resume professional and ATS-friendly.
- Use clean CSS suitable for A4 PDF.
- Do not use external images or external CSS.
...
`;
    ...
    const response = await ai.models.generateContent({
        model: "gemini-3.5-flash", // ✅ model, not models
        contents: prompt,
        config: {
            responseMimeType: "application/json",
            responseSchema: zodToJsonSchema(resumepdfSchema)
    ...
    if (!jsonContent.html) {
        throw new Error("Gemini did not generate HTML content");
    }
